...
import static goldenapple.enchapi.asm.EnchCoreContainer.RelaunchLogHelper;
            ...
            if (node instanceof MethodInsnNode){
                ...
                if(currentNode.name.equals("getItemEnchantability") || currentNode.name.equals("c")) {
                    if (node.getNext().getOpcode() == IRETURN) { //if there is an IRETURN after that
                        ...
                        toInject.add(new MethodInsnNode(INVOKESTATIC, "goldenapple/enchapi/api/ItemHooks", "getEnchantability", obfuscated ? "(Ladd;)I" : "(Lnet/minecraft/item/ItemStack;)I", false)); //Call our hook ("stack" will be given as the argument)
     ...
     /* This method calculates some logic to determine if an item is a damageable tool.
        ...
        (see ItemHooks.isItemEnchantable(ItemStack stack)*/
            if(node.getOpcode() == INVOKEVIRTUAL && !injected){ //We'll just find the first called method, override it and return. It's lazy but it works.
                ...
                toInject.add(new MethodInsnNode(INVOKESTATIC, "goldenapple/enchapi/api/ItemHooks", "isEnchantable", obfuscated ? "(Ladd;)Z" : "(Lnet/minecraft/item/ItemStack;)Z", false)); //Call our hook ("this" will be given as the argument)
